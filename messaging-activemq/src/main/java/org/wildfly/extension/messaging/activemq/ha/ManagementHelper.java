...
public class ManagementHelper {
    ...
    static OperationStepHandler checkNoOtherSibling(final String childType) {
        return new OperationStepHandler() {
            @Override
            public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
                PathAddress currentAddress = PathAddress.pathAddress(operation.get(ModelDescriptionConstants.OP_ADDR));
                PathAddress parentAddress = currentAddress.subAddress(0, currentAddress.size() - 1);
                Resource parent = context.readResourceFromRoot(parentAddress);
